...
if (!empty($Imagen)) {
    $Query = "UPDATE `bosh` SET`codigoi`='$Codigo1',`codigoii`='$Codigo2',`segmento`='$Segmento',`material`='$Material',`descripcion`='$Descripcion',`presentacion`='$Presentacion',`linea`='$Linea',`fuente`='$Fuente',`ean`='$Ean',`iva`='$Iva',`preciosiniva`='$PrecioLista',`pvppremium`='$PvpPremium',`cantidad`='$Cantidad',`thumb`='$Imagen' WHERE id = '$Id'";} else {
    $Query = "UPDATE `bosh` SET`codigoi`='$Codigo1',`codigoii`='$Codigo2',`segmento`='$Segmento',`material`='$Material',`descripcion`='$Descripcion',`presentacion`='$Presentacion',`linea`='$Linea',`fuente`='$Fuente',`ean`='$Ean',`iva`='$Iva',`preciosiniva`='$PrecioLista',`pvppremium`='$PvpPremium',`cantidad`='$Cantidad' WHERE id = '$Id'";
}

$Result = mysqli_query($conexion, $Query);

$A = $Result ? header("Location: ./boshAll.php") : 'no se pudo editar la información';
